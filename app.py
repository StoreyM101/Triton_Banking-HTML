...
triton_user_info = { # Dashboard Information
    "doej123": {
...
def login(): # Define Login Function
    if request.method == "POST": # If the request method is a HTTP POST request, it means that the user submitted a form.
        ...
        if username in triton_user_info and triton_user_info[username]["password"] == password: # if input matches dictionary.
    ...
    return render_template("Triton_Login.html") # Displays login form using GET request.
...
def dashboard():
    ...
    user_data = triton_user_info.get(session["username"]) # Retrieves user data from Dictionary
    if user_data: # If Data exists
        return render_template("Triton_Dashboard.html", user=user_data) # Passes user data to Dashboard
    else:
